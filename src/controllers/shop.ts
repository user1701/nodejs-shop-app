...
export const getShopHome = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
function isPopulatedProduct(id: PopulatedDoc<IProduct & Document>): id is IProduct {
    return id != null && typeof id === "object" && "price" in id;
}

function calcTotal(items: { quantity: number; id: PopulatedDoc<IProduct & Document> }[]): number {
    ...
    User.findOne({ _id: req.session.user._id })
        ...
        .then((user) => {
            ...
            const orderItems = user.cart.items
                .filter((item): item is CartItem & { id: IProduct } => item.id != null && typeof item.id !== "string")
                .map((item) => ({
